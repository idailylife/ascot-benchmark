...
2. **`runner.py`** — `BenchmarkRunner.run_all()` orchestrates execution. For each `(case, trial)` pair, it creates a workspace, runs OpenCode via `opencode_wrapper.AsyncOpenCodeClient`, preserves output, then calls the grader. All pairs share a semaphore-based concurrency pool. Permissions are merged from defaults + suite's `opencode.json`. After each session, `_strip_delta_lines` rewrites the trial's `events.jsonl` to drop the redundant `message.part.delta` streaming chunks emitted in server mode (the final `message.part.updated` snapshot retains each part's full text); reasoning entries are kept for `ascot review`.
...
4. **`graders.py`** — Grading orchestration. For each case, runs the deterministic `test_script` (if set) via `verifiers.run_test_script`, then runs the LLM judge over any `expectations`, and concatenates both result lists. The judge is a second OpenCode session in an isolated workspace with `events.jsonl` + `output/` files and a structured prompt; it parses the JSON verdict (`_parse_judge_response`) and scores 0 on parse failure. `_copy_events_for_judge` builds the judge's copy of `events.jsonl`, dropping both reasoning entries and `message.part.delta` chunks to keep judge context small. Accepts an optional `grading_model` to override the judge's model (priority: `grading_model` > `default_model` > opencode default). A case with only `test_script` (no `expectations`) skips the judge entirely — no judge cost. Also contains `review_case()` for the `ascot review` subcommand — a diagnostic agent that analyzes failed cases across trials, comparing successful vs failed trials to identify root causes.

5. **`verifiers.py`** — Deterministic pytest-based grading. `run_test_script(workspace_dir, script_path, timeout_s)` runs `python -m pytest <script>` with `cwd=workspace_dir`, parses the junit XML, and returns one `ExpectationResult` per pytest test (1 point each by default; a `@pytest.mark.score(N)` marker overrides the weight, surfaced via the bundled `ascot._score_plugin` pytest plugin; skipped tests excluded). `script_path` may be a plain file or a pytest nodeid `file.py::test_name`. All edge cases (missing file, collection error, timeout, parse error) collapse to a single failed `ExpectationResult` so the case still gets a non-zero `max_score` and the failure reason is visible in the report.
